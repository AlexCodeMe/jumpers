import { createNotificationBox } from "../ui/notification-box";
import { makeBlink } from "./shared-logic";
import { state, stateProps } from "../state/global-state";

export function createBoss(k, initialPos) {
  return k.make([
    k.pos(initialPos),
    ...
    k.opacity(1),
    {
      ...
      setEvents() {
        ...
        this.on("explode", () => {
          ...
          k.play("boom");
          this.play("explode");
          state.set(stateProps.isBossDefeated, true);
          state.set(stateProps.isDoubleJumpUnlocked, true);
