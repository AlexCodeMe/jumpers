...
export function setBackgroundColor(k, hexColor) {
  k.setBackground(hexColor);
}
  ...
  for (const collider of colliders) {
    ...
    if (collider.name === "boss-barrier") {
      ...
      bossBarrier.onCollide("player", async (player) => {
        ...
        if (curState.isBossDefeated) {
          state.set(stateProps.playerInBossFight, false);
          bossBarrier.deactivate(player.pos.x);
          return;
        }

        if (curState.playerInBossFight) {
          return;
        }
      ...
      bossBarrier.onCollideEnd("player", () => {
        const curState = state.current();
        if (curState.playerInBossFight || curState.isBossDefeated) {
          return;
        }

        state.set(stateProps.playerInBossFight, true);
